        ...
        <div class="header-content">
          <div class="header-left">
            ...
            <div class="header-text">
              <h1 class="header-title">Story Practice</h1>
      ...
      <div class="main-content">
        ...
        <div class="progress-container">
          <div class="progress-header">
            ...
            <span class="progress-count"
              >{{ currentWordIndex + 1 }}/{{ loadedMiscues.length }}</span
            >
          </div>
        ...
        <div class="word-card">
          <div class="word-card-header">
            <h2 class="practice-label">Practice this word from the story:</h2>
          </div>

          <div class="word-display">
            {{ currentStoryMiscue?.expected_word || "" }}
          </div>
          ...
          <!-- Context from story -->
          <div v-if="currentStoryMiscue?.context" class="story-context">
            <p class="context-label">In the story:</p>
            <p class="context-text">"{{ currentStoryMiscue.context }}"</p>
          </div>
...
const loadedMiscues = ref([]);
const loadedSessionId = ref(null);
const loadedStoryTitle = ref("");

// Load from sessionStorage on component creation
try {
  ...
  console.error("Failed to load story miscue data:", error);
}
...
// Computed properties
const currentStoryMiscue = computed(() => {
  return loadedMiscues.value[currentWordIndex.value] || null;
});

const progressPercentage = computed(() => {
  if (loadedMiscues.value.length === 0) return 100;
  return Math.round(((currentWordIndex.value + 1) / loadedMiscues.value.length) * 100);
});
...
const processPendingRepetitions = async () => {
  ...
  console.log("📋 processPendingRepetitions called:", {
    profileId: profile.value?.id,
    userId: userId,
    sessionId: loadedSessionId.value,
...
const checkWord = async (spoken) => {
  const expectedWord = currentStoryMiscue.value?.expected_word;
  ...
  if (similarity >= 0.6) {
    ...
    playWordFeedback(true); // 🎵 Play correct word sound
    correctedWords.value.add(currentWordIndex.value);
    lastAttemptCorrect.value = true;

    // Record the successful repetition
    const userId = await getUserId();
    if (userId && loadedSessionId.value) {
    ...
    playWordFeedback(false); // 🎵 Play incorrect word sound
    lastAttemptCorrect.value = false;

    // Record the failed repetition
    const userId = await getUserId();
    if (userId && loadedSessionId.value) {
...
const closeFeedbackModal = () => {
  ...
  if (lastAttemptCorrect.value) {
    ...
    setTimeout(() => {
      if (currentWordIndex.value < loadedMiscues.value.length - 1) {
        nextWord();
      } else {
        finishReview();
      }
...
const recordRepetitionMiscue = async (expectedWord, actualReading, wasCorrect) => {
  try {
    ...
    // Debug logging
    console.log("🔍 Attempting to record story repetition miscue with:", {
      userId: userId,
      profileId: profile.value?.id,
      sessionId: loadedSessionId.value,
      expectedWord,
    ...
    console.log(
      `📝 Recording story repetition miscue #${currentCount}: "${actualReading}" for "${expectedWord}" - ${
        wasCorrect ? "Correct" : "Incorrect"
      }`
    );

    const { error } = await supabase.from("reading_miscues").insert({
      reading_session_id: loadedSessionId.value,
      user_id: userId,
    ...
    if (error) {
      console.error("Error recording story repetition miscue:", error);
    } else {
      console.log(`✅ Story repetition miscue attempt recorded successfully`);
    }
  } catch (error) {
...
const nextWord = () => {
  if (currentWordIndex.value < loadedMiscues.value.length - 1) {
    ...
    // Initialize repetition count for the new word
...
const initializeWordRepetition = async () => {
  ...
  if (!wordRepetitions.value.has(currentIndex)) {
    ...
    if (userId && loadedSessionId.value) {
      ...
      await recordRepetitionMiscue(
        expectedWord,
        currentStoryMiscue.value?.actual_reading || expectedWord,
        false
      );
    } else {
      console.log(
      ...
      pendingRepetitions.value.push({
        expectedWord,
        actualReading: currentStoryMiscue.value?.actual_reading || expectedWord,
        wasCorrect: false,
        wordIndex: currentIndex,
      });
    }
...
// Skip review
const skipReview = () => {
  console.log("⏩ Story miscue review skipped");
...
// Finish review
const finishReview = () => {
  console.log("✅ Story miscue review completed");
  const stats = {
    totalWords: loadedMiscues.value.length,
    correctedWords: correctedWords.value.size,
    percentageCorrected:
      loadedMiscues.value.length > 0
        ? Math.round((correctedWords.value.size / loadedMiscues.value.length) * 100)
        : 0,
  };
...
// Get capybara expression based on progress
...
const getErrorTypeMessage = (errorType) => {
  const messages = {
    substitution: "You read a different word",
    omission: "You skipped this word",
    insertion: "You added an extra word",
    repetition: "You repeated this word",
    mispronunciation: "This word was mispronounced",
  };

  return messages[errorType] || "Let's practice this word";
};
...
onMounted(async () => {
  ...
  console.log("📚 Story Title:", loadedStoryTitle.value);

  // Check if we have miscue data
  if (loadedMiscues.value.length === 0) {
    console.warn("⚠️ No miscues found - redirecting to stories");
    router.push("/tabs/stories");
    return;
  }
  ...
  // Process any pending repetitions
